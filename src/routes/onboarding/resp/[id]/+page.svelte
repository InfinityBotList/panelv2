<script lang="ts">
	...
	const fetchOnboardingResponse = async () => {
		...
		if (!resp.ok) {
			...
			throw new Error(
				err?.toString() || 'An unknown error occurred while loading the onboarding response'
			);
		}
		...
		<article class="p-4">
			...
			{#each resp?.questions || [] as question, i}
				...
				{:else if question.data.multiple_choice}
					...
					<small class="text-white mb-1"
						>Available Options: {question?.data?.multiple_choice.join(', ')}</small
					>
				{/if}
			{/each}
			...
			{#if resp?.verdict}
				...
				<ObjectRender object={resp?.verdict} />
			{/if}
