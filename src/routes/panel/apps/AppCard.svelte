<script lang="ts">
    ...
    const approveDenyAction = async () => {
        ...
        if (!res.ok) {
            ...
            throw new Error(`Failed to approve application: ${err?.message?.toString() || err || "Unknown error"}`);
        }
...
<Card>
    ...
    <svelte:fragment slot="display-name">{app?.user?.display_name} ({app?.user?.username})</svelte:fragment>
...
{#if showActionsModal}
    <Modal bind:showModal={showActionsModal}>
        ...
        <p class="break-all"><strong>App User:</strong> {app?.user?.username} [{app?.user?.display_name}]</p>
