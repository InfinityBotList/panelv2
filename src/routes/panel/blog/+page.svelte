<script lang="ts">
	...
	import type { BaseSchema, Capability, CustomAction, CustomActionFetch, Entry, FieldFetch, Schema } from "../../../components/admin/types";
    ...
    class BlogSchema implements BaseSchema<BlogPost>, Schema<BlogPost> {
        ...
        fields: FieldFetch<BlogPost> = [
            ...
            async (cap) => {
				...
				return newField("user_id", "User ID", "The ID of the user who created the blog post", true, true) // user_id is not editable
			},
            async (cap) => {
                if (cap == "create") return null // created_at is not available in create
				return newField("created_at", "Created At", "The time the blog post was created at", true, true) // user_id is not editable
			},
