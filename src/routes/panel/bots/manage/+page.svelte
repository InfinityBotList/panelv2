...
{:then meta}
	<div class="mt-10">
        ...
        <InputText id="search" label="Search for a bot?" minlength={0} showErrors={false} bind:value={searchQuery} placeholder="Search for a bot to manage" />
                ...
                {#each searchResults as bot, i}
                    <Card>
                        ...
                        <span slot="post-slot" class="block mt-5 text-md tracking-tight my-2">
                            ...
                            <ManageAction bot={bot} actions={meta?.actions} />
                        </span>
                    </Card>
                {/each}
            </Column>
        {:else}
            <p class="font-semibold text-xl text-red-500">There are no bots matching your query! Try making another search?</p>
        {/if}
	</div>
{:catch err}
