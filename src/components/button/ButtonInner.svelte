...
<div 
    ...
    aria-live="polite"
>
    <div class={(state?.iconClass ? (state?.iconClass) + " " : "") + ' px-4 py-2 rounded-l-lg text-white align-middle'}>
        <Icon
            icon={icon}
            inline={true}
            class={'inline-flex text-white align-items items-center justify-center' + (icon == 'mdi:loading' ? ' animate-spin' : '')}
        />
    </div>
    <div class={(state?.colorClass ? (state?.colorClass) + " " : "") + ' w-full px-4 py-2 rounded-r-lg text-white text-left'}>
